...
const css = `
  body.financial-only-role .sidebar nav button:not(.financial-projects-nav-button) {
    display: none !important;
  }

  body.financial-only-role main.workspace > *:not(.topbar):not(.app-message):not(.financial-projects-page) {
    display: none !important;
  }
  ...
  body.financial-readonly .financial-row-actions {
    display: none !important;
  }
...
function rewriteRoleNotice(profile = currentProfile) {
  ...
  if (notice) {
    ...
    notice.textContent = roleNotices[role] || roleNotices.viewer;
...
function ensureFinancialOnlyView(profile = currentProfile) {
  ...
  if (financialButton && !document.querySelector(".financial-projects-page")) {
    financialButton.click();
  }
}
...
function enhanceUserRoleSelectors() {
  loadProfiles().then((profiles) => {
    ...
    document.querySelectorAll("table tbody tr").forEach((row) => {
      ...
      if (status) status.textContent = roleLabel(profile.role);

      const select = row.querySelector("select");
      if (!select) return;

      const currentValue = profile.role || "viewer";
      const optionHtml = roleOptions.map(([value, label]) => `<option value="${value}">${label}</option>`).join("");
      if (select.dataset.roleOptionsEnhanced !== "true") {
        select.innerHTML = optionHtml;
        select.dataset.roleOptionsEnhanced = "true";
      }

      select.value = roleOptions.some(([value]) => value === currentValue) ? currentValue : "viewer";
      ...
      if (!row?.id) {
        ...
        td.innerHTML = `
          <div class="financial-row-actions">
            <button type="button" class="financial-edit">Edit</button>
            <button type="button" class="financial-delete">Delete</button>
          </div>
        `;
  ...
  backdrop.innerHTML = `
    <form class="financial-modal">
      ...
      <div class="financial-form-grid">
        ...
        <label>Status<select name="status">${statusOptions.map(([value, label]) => `<option value="${value}" ${row.status === value ? "selected" : ""}>${label}</option>`).join("")}</select></label>
...
async function deleteFinancialRow(row) {
  const ok = window.confirm(`Delete financial project "${row.project_name}"?`);
  if (!ok) return;
...
function startAccessEnhancer() {
  ...
  loadCurrentProfile()
    .then((profile) => {
      applyBodyAccessClasses(profile);
      rewriteRoleNotice(profile);
      ensureFinancialOnlyView(profile);
      enhanceFinancialTableActions();
    })
    .catch(() => null);
