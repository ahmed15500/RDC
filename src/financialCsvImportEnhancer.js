...
const headerAliases = {
  project_code: ["project_code", "project code", "code", "project id", "project_id", "id", "project no", "project number", "reference", "ref", "كود المشروع", "رقم المشروع"],
...
const aliasLookup = Object.fromEntries(
  Object.entries(headerAliases).flatMap(([canonical, aliases]) => aliases.map((alias) => [normalizeHeaderKey(alias), canonical])),
);
...
function downloadTemplate() {
  const example = ["GWS-2025-001", "GWS-SENSE", "planning", "Environment & Natural Resources", "HU", "2025", "2027", "5277009.87", "120000", "2025", "Planning status is accepted. Total budget is counted once."];
...
function normalizeStatus(value) {
  ...
  return statusAliases[raw] || statusAliases[normalized] || normalized || "running";
}
...
function validateRows(parsed) {
  ...
  const mappings = headers.filter((header) => header.original && header.original !== header.canonical).map((header) => `${header.original} → ${header.canonical}`);
  if (!canonicalHeaders.includes("project_name")) errors.push("Missing required columns: project_name. Project code, years, annual amount, notes, sector, entity, and status can be generated or defaulted.");
  ...
  parsed.slice(1).forEach((line, index) => {
    ...
    const annualAmount = parseAmount(record.annual_amount_eur);
...
function handleFileSelection(event) {
  ...
  reader.onload = () => {
    try { showPreview(excel ? validateExcel(reader.result) : validateCsv(String(reader.result || ""))); }
    catch (error) { showPreview({ rows: [], errors: [`Could not read the selected ${excel ? "Excel" : "CSV"} file: ${error.message}`], totalRows: 0, mappings: [], sourceType: excel ? "Excel" : "CSV" }); }
  };
